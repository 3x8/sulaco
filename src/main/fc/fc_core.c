...
void updateArmingStatus(void) {
  if (ARMING_FLAG(ARMED)) {
    ...
    if (!isUsingSticksForArming()) {
      ...
      bool ignoreThrottle = feature(FEATURE_3D) && !IS_RC_MODE_ACTIVE(BOX3D) && !flight3DConfig()->switched_mode3d && !(getArmingDisableFlags() & ~(ARMING_DISABLED_ARM_SWITCH | ARMING_DISABLED_THROTTLE));
...
bool processRx(timeUs_t currentTimeUs) {
  ...
  if (ARMING_FLAG(ARMED) && feature(FEATURE_MOTOR_STOP) && !STATE(FIXED_WING) && !feature(FEATURE_3D) && !isAirmodeActive() && !FLIGHT_MODE(GPS_RESCUE_MODE)) {
    if (isUsingSticksForArming()) {
...
static FAST_CODE void subTaskPidController(timeUs_t currentTimeUs) {
    ...
    DEBUG_SET(DEBUG_PIDLOOP, 1, micros() - startTime);
      ...
      if (ARMING_FLAG(ARMED) && !STATE(FIXED_WING) && pidConfig()->runaway_takeoff_prevention && !runawayTakeoffCheckDisabled && !flipOverAfterCrashMode && !runawayTakeoffTemporarilyDisabled && (!feature(FEATURE_MOTOR_STOP) || isAirmodeActive() || (calculateThrottleStatus() != THROTTLE_LOW))) {
