...
PG_RESET_TEMPLATE(pilotConfig_t, pilotConfig, .name = { 0 });
...
static void validateAndFixConfig(void) {
  ...
  if (!rcSmoothingIsEnabled() ||
    (rxConfig()->rcInterpolationChannels != INTERPOLATION_CHANNELS_RPY && rxConfig()->rcInterpolationChannels != INTERPOLATION_CHANNELS_RPYT)) {
    for (unsigned i = 0; i < MAX_PROFILE_COUNT; i++) {
  ...
  if (featureConfigured(FEATURE_3D) || !featureConfigured(FEATURE_GPS)
