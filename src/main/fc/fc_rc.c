...
// Determine a cutoff frequency based on filter type and the calculated average rx frame time
...
// Preforms a reasonableness check on the rx frame time to avoid bad data skewing the average.
...
// Initialize or update the filters base on either the manually selected cutoff, or the auto-calculated cutoff frequency based on detected rx frame rate.
...
// Determine if we need to caclulate filter cutoffs. If not then we can avoid examining the rx frame times completely
