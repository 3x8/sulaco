...
#define repeatAfter(t) { rcDelayMs -= (t); doNotRepeat = false; }

void processRcStickPositions() {
  static int16_t rcDelayMs;       // time the sticks are maintained
  static uint8_t rcSticks;        // hold sticks position for command combos
  static uint8_t rcDisarmTicks;   // an extra guard for disarming through switch to prevent that one frame can disarm it
  ...
  if (rcSticks == THR_LO + YAW_LO + PIT_LO + ROL_CE) {
    gyroStartCalibration(false);  // GYRO calibration
